...
public class StudyFragment extends BaseFragment implements View.OnClickListener {
    ...
    public void openAddClassBottomSheet() {
        ...
        AddClassBottomSheetDialog addClassBottomSheetDialog = new AddClassBottomSheetDialog(StudyFragment.this.getActivity(), result -> {
            if (result == 1) { //success
                Toast.makeText(StudyFragment.this.getActivity(), "Task Completed Success", Toast.LENGTH_SHORT).show();
            } else { // Failure
                Toast.makeText(StudyFragment.this.getActivity(), "Task Failed", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    public class LoadLocalClassList extends AsyncTask<Void, Void, List<ClassEntity>> {
        ...
        protected List<ClassEntity> doInBackground(Void... voids) {
            ...
            if (prefData.equals("")) {
                ...
                jsonArray = Utility.readAssetsJsonFile(StudyFragment.this.getActivity(), "classes.json");
            } else {
